...
then
       ...
       echo ""
       wine "$WINEPREFIX/../INSTALL_rFactor/rFactorDAQPluginSetup_1.3.2.exe" 2>/dev/null 1>/dev/null
       # switch log file format to .csv (the default is MoTec)
       cp "$WINEPREFIX/../INSTALL_rFactor/DataAcquisitionPlugin.ini" "$WINEPREFIX/drive_c/Program Files (x86)/rFactor"
else
