...
if [ ! -d $WINEPREFIX/../INSTALL_rFactor ]
then
        clear
        echo "$WINEPREFIX/../INSTALL_rFactor not found.  Download the directory before proceeding."; echo ""
        echo "See the file 'additionalGames.txt' for information about where to download the"
        echo "INSTALL_rFactor add-on package. Unpack this file into the INSTALL directory."
...
wine $WINEPREFIX/../INSTALL_rFactor/jre-8u291-windows-i586.exe /s 2>/dev/null 1>/dev/null
echo "Installing AI improvement plugin v. 1.21" 
rsync -a "$WINEPREFIX/../INSTALL_rFactor/Plugins/"   "$WINEPREFIX/drive_c/Program Files (x86)/rFactor/Plugins/"
echo ""
