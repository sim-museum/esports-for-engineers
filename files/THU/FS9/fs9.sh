#launcher.py won't start this script unless commented out
...
if [ -f "$WINEPREFIX/drive_c/Program Files (x86)/Microsoft Games/Flight Simulator 9/fs9.exe" ]
then
		clear
...
mv "$WINEPREFIX/../../../tar/Microsoft-Flight-Simulator-2004-A-Century-of-Flight_Win_EN_OEM-version-Updated-to-91.zip" $WINEPREFIX/../INSTALL 2>/dev/null 1>/dev/null

if [ ! -f "$WINEPREFIX/../INSTALL/Microsoft-Flight-Simulator-2004-A-Century-of-Flight_Win_EN_OEM-version-Updated-to-91.zip" ]
then
	clear
...
#	winetricks winamp 2>/dev/null 1>/dev/null
...
unzip "Microsoft-Flight-Simulator-2004-A-Century-of-Flight_Win_EN_OEM-version-Updated-to-91.zip" 2>/dev/null 1>/dev/null
		printf "To install Flight Simulator 9, run the following command in a terminal:\n\nsudo mount -o loop \"$WINEPREFIX/../INSTALL/Microsoft Flight Simulator 2004/FS_OEM.mdf\" \"$WINEPREFIX/../INSTALL/isoMnt\"\n\nThen run this script again.\n"
		exit 0
fi
