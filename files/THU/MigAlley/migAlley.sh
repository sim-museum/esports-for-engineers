...
if [ "$numMonitors" -ne 2 ]; then
    ...
    echo "To continue with your current monitor setup, press Enter\n"
    read replyString
fi
...
if [ ! -f "/usr/bin/winetricks" ]; then
    ...
    echo "during Mig Alley installation.  If using Ubuntu 22.04 LTS install winetricks via:"
...
wine "$MA_ISO/setup.EXE" 2>/dev/null 1>/dev/null
clear
...
wine "$INSTALL_DIR/Mig-Alley_Patch_Win_EN_Patch-123/MIG123.EXE" 2>/dev/null 1>/dev/null
wine "$INSTALL_DIR/bdg_migalley_0.85f/BDG_MiGAlley_0.85F.exe" 2>/dev/null 1>/dev/null
